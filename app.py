...
if uploaded_file:
    # Очищаем старые данные при загрузке НОВОГО файла
    ...
    if not st.session_state['title_info']:
        with st.spinner("Критический анализ реквизитов..."):
            # МАКСИМАЛЬНО ЖЕСТКИЙ ПРОМПТ
            extraction_prompt = f"""ВНИМАНИЕ: Извлеки данные СТРОГО из приложенного текста. 
            Игнорируй любые фамилии, которые ты знал раньше.
            ТЕКСТ КОНТРАКТА: {contract_text[:8000]}
            
            Выдай JSON:
            {{
              "company": "Найти в преамбуле после слова Исполнитель",
              "director": "Найти ФИО сразу после слов 'в лице Директора'",
              "contract_no": "Номер из первой строки",
              "contract_date": "Дата из первой строки",
              "project_name": "Предмет контракта"
            }}
            ВАЖНО: Если в тексте написано 'Скиба', пиши 'Скиба'. Если 'Иванов', пиши 'Иванов'."""
