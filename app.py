        ...
        with st.spinner("Анализ титульных данных..."):
            try:
                res = client_ai.chat.completions.create(
                    ...
                    messages=[{
                        "role": "user",
                        "content": f"""Ты - точный экстрактор.  
                        Извлеки данные СТРОГО из текста.  
                        НЕ придумывай, НЕ перефразируй, НЕ добавляй ничего от себя.  
                        Если поле не найдено - пиши "-".  
                        Верни ТОЛЬКО чистый JSON. Никакого текста вне JSON.
                        
                        Ключи:
                        - contract_no: номер контракта из первой строки (пример: 10/25-ГК)
                        - contract_date_raw: дата контракта как написана (пример: __ _____________ 2025 г.)
                        - ikz: ТОЛЬКО 36-значное число после слов "ИКЗ", "Идентификационный код закупки" или похожих - найди именно его, игнорируй другие номера
                        - project_name: полное наименование предмета контракта / услуги из пункта 1.1 или шапки после номера контракта, с большой буквы
                        - customer_org: полное название Заказчика (обычно сразу после города или в шапке)
                        - customer_post: ТОЛЬКО должность подписанта Заказчика, которая стоит ПЕРЕД ФИО (Министр, Генеральный директор и т.п.) - БЕЗ слов "на основании", "в лице" и т.д.
                        - customer_fio_raw: ФИО подписанта Заказчика как написано рядом с должностью
                        - executor_org: полное название Исполнителя
                        - executor_post: ТОЛЬКО должность подписанта Исполнителя ПЕРЕД ФИО (Генеральный директор, Директор и т.п.)
