...
if uploaded_file:
    ...
    if submitted:
        with st.spinner("Генерация отчета по эталону..."):
            # Берем структуру из таблицы на основе определенного ИИ типа
            selected_row = df_etalons[df_etalons["Тип проекта"] == meta['type']].iloc[0]
            struct = selected_row.get("ЭТАЛОННАЯ СТРУКТУРА", "Стандартная")
            
            sys_msg = f"Ты юрист. Напиши отчет в прошедшем времени. Используй эталонную структуру: {struct}. Отрази все пункты ТЗ как выполненные."
...
if 'report_buffer' in st.session_state:
    st.download_button("📥 Скачать Отчет", st.session_state['report_buffer'], "Report.docx")
