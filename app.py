...
with col1:
    ...
    with t_tab2:
        manual_context = st.text_area("Вставьте текст первой страницы", height=200, key="manual_title")
        if manual_context:
            context = manual_context

    # ОДНА КНОПКА ДЛЯ ВСЕХ ВИДОВ ВВОДА
    if st.button("Сформировать Титульный лист", key="btn_title"):
        if not context:
            ...
            with st.spinner("ИИ извлекает реквизиты..."):
                ...
                res = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system", "content": """Ты выполняешь функцию парсера данных для государственных контрактов. Работай ТОЛЬКО с тем текстом, который передан в текущем запросе. 
                            Из предоставленного текста контракта извлеки параметры и верни их исключительно в формате JSON, без лишних пояснений.
                            Запрещено использовать любые данные из предыдущих диалогов или внешних знаний. 
                            Внимательно проверь, что ИКЗ — это именно код закупки, а не номер счета или ИНН. ИКЗ содержит до 36 цифр. 
...
with col2:
    ...
    with r_tab2:
        manual_tz = st.text_area("Вставьте текст ТЗ", height=200, key="manual_tz")
        if manual_tz:
            raw_tz = manual_tz

    # ОДНА КНОПКА ДЛЯ ВСЕХ ВИДОВ ВВОДА
    if st.button("Сформировать Рукописный отчет", key="btn_report"):
        if not raw_tz:
    ...
    if "file_report_only" in st.session_state:
        st.download_button("📥 Скачать Отчет", st.session_state.file_report_only, "Report_Only.docx")
